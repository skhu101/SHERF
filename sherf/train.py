...
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=0), required=True)
...
@click.option('--resume_kimg',  help='Resume kimg', metavar='INT',                              type=click.IntRange(min=0), default=0, show_default=True)
...
# SHERF model config
@click.option('--use_1d_feature', help='Use 1d latent code feature', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--use_2d_feature', help='Use 2d image feature', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--use_3d_feature', help='Use 3d volume feature', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--white_back', help='Use vertex feature', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--recons_loss', help='Use vertex feature', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--sample_obs_view', help='Use vertex feature', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--fix_obs_view', help='Use vertex feature', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--use_sr_module', help='Use gan loss in the second stage', metavar='BOOL',  type=bool, required=False, default=True)
@click.option('--use_trans', help='Use transformer feature fusion', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--use_nerf_decoder', help='Use NeRF decoder', metavar='BOOL',  type=bool, required=False, default=False)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, w_dim=512, use_1d_feature=opts.use_1d_feature, use_2d_feature=opts.use_2d_feature, use_3d_feature=opts.use_3d_feature, use_trans=opts.use_trans, use_NeRF_decoder=opts.use_nerf_decoder, mapping_kwargs=dnnlib.EasyDict())
    ...
    elif opts.cfg == 'THuman':
        dataset_kwargs = dnnlib.EasyDict(class_name='training.THuman_dataset.THumanDatasetBatch', data_root=opts.data, split='train', multi_person=True, num_instance=90, poses_start=0, poses_interval=1, poses_num=20, image_scaling=opts.neural_rendering_resolution_initial/512, white_back=opts.white_back, sample_obs_view=opts.sample_obs_view, fix_obs_view=opts.fix_obs_view)
        # dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        c.training_set_kwargs = dataset_kwargs
        c.training_set_kwargs.resolution = 512
        dataset_name = opts.cfg
    elif opts.cfg == 'HuMMan':
        dataset_kwargs = dnnlib.EasyDict(class_name='training.HuMMan_dataset.HuMManDatasetBatch', data_root=opts.data, split='train', multi_person=True, num_instance=317, poses_start=0, poses_interval=6, poses_num=17, image_scaling=1/3, sample_obs_view=opts.sample_obs_view, fix_obs_view=opts.fix_obs_view)
        c.training_set_kwargs = dataset_kwargs
        c.training_set_kwargs.resolution = 512
        dataset_name = opts.cfg
    elif opts.cfg == 'zju_mocap':
        dataset_kwargs = dnnlib.EasyDict(class_name='training.NeuBody_dataset.NeuBodyDatasetBatch', data_root=opts.data, split='train', multi_person=True, num_instance=6, poses_start=0, poses_interval=5, poses_num=100, image_scaling=opts.neural_rendering_resolution_initial/1024, sample_obs_view=opts.sample_obs_view, fix_obs_view=opts.fix_obs_view)
        # dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        c.training_set_kwargs = dataset_kwargs
        c.training_set_kwargs.resolution = 512
        dataset_name = opts.cfg
    ...
    c.D_kwargs.epilogue_kwargs.mbstd_group_size = 1 #opts.mbstd_group
    ...
    c.recons_loss = opts.recons_loss
    ...
    # Base configuration.
    c.ema_kimg = 10 #c.batch_size * 10 / 32
    ...
    if opts.cfg == 'THuman' or opts.cfg == 'zju_mocap' or opts.cfg == 'RenderPeople' or opts.cfg == 'HuMMan':
        rendering_options.update({
            'depth_resolution': 48, # number of uniform samples to take per ray.
            'depth_resolution_importance': 0, # number of importance samples to take per ray.
        })
    elif opts.cfg == 'ffhq':
        rendering_options.update({
            'depth_resolution': 48, # number of uniform samples to take per ray.
    ...
    c.loss_kwargs.gpc_reg_prob = opts.gpc_reg_prob if opts.gen_pose_cond else None # 0.8
    c.loss_kwargs.gpc_reg_fade_kimg = opts.gpc_reg_fade_kimg # 1000
    c.loss_kwargs.dual_discrimination = True
    c.loss_kwargs.neural_rendering_resolution_initial = opts.neural_rendering_resolution_initial # 64
    c.loss_kwargs.neural_rendering_resolution_final = opts.neural_rendering_resolution_final # 128
    c.loss_kwargs.neural_rendering_resolution_fade_kimg = opts.neural_rendering_resolution_fade_kimg # 1000
    c.G_kwargs.sr_num_fp16_res = opts.sr_num_fp16_res # 4

    c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax, fused_modconv_default='inference_only')

    c.loss_kwargs.style_mixing_prob = opts.style_mixing_prob # 0.0
